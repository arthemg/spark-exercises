...
def loadMovieNames():
    movieNames = {}
    with open("DataSets/ml-100k/u.item", encoding="ascii", errors='ignore') as file:
        for line in file:
            fields = line.split('|')
...
if len(sys.argv) > 1:
    ...
    # NOTE Replaces below lambda with a method
    #      filterResults = movieSimilarities.filter(lambda pairSim: (pairSim[0][0] == movieID or pairSim[0][1] == movieID)
